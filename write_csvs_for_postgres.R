...
# For testing pull out a single division
sites <- sites[division_name == 'Africa Field Division']
...
pixelsbysite <- pixelsbysite[site_id 
...
# TODO: check the new_or_continued column and fix for multiple different values 
# - exclude "Inactive Due to COVID" 

partied_data %>%
    group_by(site_id, id) %>%
    slice(which.max(coverage_fraction)) %>%
    group_by(country_id, reporting_year, new_or_continued) %>%
    mutate(
...
# TODO: groupings: scapes, divisions, sites




ecosystem_data %>%
    group_by(country_id, reporting_year, new_or_continued, ecosystem) %>%
    summarise(
        ttl_ha_ic=sum(calc_ha_ic)
    ) %>%
...
# The above could be run with slight changes to produce the Division, Scape, and Site data files. In each case you would replace the country_id grouping with the corresponding id.
...
(
	...
	FROM crosstab(
		'SELECT country_id, new_or_continued, reporting_year, ecosystem AS ecosystemid, SUM(calc_tstor) AS ttl_tstor
		FROM m_countrypixels
		WHERE (ecosystem is not null AND ecosystem > 0)
		AND new_or_continued = ''Continued this year''
		GROUP BY country_id, new_or_continued, reporting_year, ecosystem
	...
	AS ct(country_id text, new_or_continued text, reporting_year text, "1" double precision, "2" double precision, "3" double precision, "4" double precision, "5" double precision, "6" double precision, "7" double precision, "8" double precision)
	UNION ALL
	SELECT
		ct.country_id,
		ct.new_or_continued,
	...
	FROM crosstab(
		...
		FROM m_countrypixels
		WHERE (ecosystem is not null AND ecosystem > 0)
		AND new_or_continued = ''New this year''
		GROUP BY country_id, new_or_continued, reporting_year, ecosystem
		ORDER BY 1, 2'::text, 'VALUES (''1''::double precision),(''2''::double precision),(''3''::double precision),(''4''::double precision),(''5''::double precision),(''6''::double precision),(''7''::double precision),(''8''::double precision)'::text)
	AS ct(country_id text, new_or_continued text, reporting_year text, "1" double precision, "2" double precision, "3" double precision, "4" double precision, "5" double precision, "6" double precision, "7" double precision, "8" double precision)
) AS csquery ON csquery.country_id = cp.country_id AND csquery.new_or_continued = cp.new_or_continued AND csquery.reporting_year = cp.reporting_year
